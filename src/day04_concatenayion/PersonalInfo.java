package day04_concatenayion;

public class PersonalInfo {
    public static void main(String[] args) {

        String firstName = "Aaron",
                lastName = "Daniel",
                from = "United Kingdon",
                favoriteMusic = "Rock music",
                favoriteBook = "Data Struktures in Java",
                favoriteTvShow = "The Walking Dead",
                favoriteColor = "Blue";
        char grender = 'M';
        int age = 32;
        boolean isMarried = true,
                isEmployee = false;

        System.out.println("firstName = " + firstName);
        System.out.println("lastName = " + lastName);
        System.out.println("grender = " + grender);
        System.out.println("age = " + age);
        System.out.println("from = " + from);
        System.out.println("favoriteMusic = " + favoriteMusic);
        System.out.println("favoriteBook = " + favoriteBook);
        System.out.println("favoriteTvShow = " + favoriteTvShow);
        System.out.println("favoriteColor = " + favoriteColor);
        System.out.println("isMarried = " + isMarried);
        System.out.println("isEmployee = " + isEmployee);



    }

}
/* Create a class named PersonalInfo, and declare thr variables with appropriate for the followings:
        1. first name
        2. last name
        3. gender
        4. age
        5. from
        6. favorite music
        7. favorite book
        8. favorite show
        9. favorite color
        10. married status
        11. employee status

 */