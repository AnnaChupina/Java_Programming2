...
public class IPhone {
    ...
    public void setColor(String color) {
        String[] colors = {"Black", "White", "Silver", "Gold", "Blue", "Pink"};
        if (Arrays.asList(colors).contains(color)) {


            this.color = color;
        } else {
            System.err.println("Invalid color: " + color + " color of the phonr can only be: " + Arrays.toString(colors));
            System.exit(1);
        }
    }
    ...
    public boolean equals(Object obj) {
        if (!(obj instanceof IPhone)){   // if the given object is Iphone
            ...
            if(model.equals(((IPhone)obj).model)){  // if the model of the Iphine is equal to the given Iphone's model
